...
public partial class ConsoleView : UserControl
{
    ...
    public ConsoleView()
    {
        ...
        SizeChanged += (_, _) => ApplyResponsiveForm();
        AttachedToVisualTree += (_, _) => ApplyResponsiveForm();
    ...
    private void ApplyResponsiveForm()
    {
        if (SendGrid is null || Bounds.Width <= 0) return;
        bool narrow = Bounds.Width < ResponsiveForm.NarrowWidth;
        if (narrow == _narrow && SendGrid.ColumnDefinitions.Count > 0) return;
        _narrow = narrow;

        ResponsiveForm.Apply(SendGrid, narrow, "86,*,Auto",
            new Control[] { BroadcastLabel, Message, SayButtons },
            new Control[] { CommandLabel, Command, RunButtons },
            new Control[] { MapLabel, Maps, MapButtons });
    }
    ...
    private async Task SendConsoleCommandAsync()
    {
        ...
        {
            await Prompt.NoticeAsync(this, "Chivalry RCON",
                "That command needs a player name. Replace <name>, or use "
                + $"\"Console command on player…\" from the roster's {Gestures.Secondary} menu.");
            return;
        }
