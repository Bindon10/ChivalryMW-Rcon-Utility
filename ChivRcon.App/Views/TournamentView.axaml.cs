...
///
/// Every button here is a BangMod opcode. Against a vanilla server they all do nothing --
...
public partial class TournamentView : UserControl
{
    ...
    {
        if (!await Prompt.ConfirmAsync(_owner, "Restart match",
    ...
    {
        var team = await Prompt.TextAsync(_owner, "End match",
            "Winning team (0 = Agatha, 1 = Mason, -1 = draw):", "-1");
        if (team is null || !int.TryParse(team, out int teamId)) return;

        var reason = await Prompt.TextAsync(_owner, "End match", "Reason shown to players:",
